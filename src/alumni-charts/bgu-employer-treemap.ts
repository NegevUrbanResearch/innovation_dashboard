...
/** Internal marker for employers merged into the localized "Other" tile. */
const OTHER_EMP_KEY = "__bgu_treemap_other__";
const TOP_EMPLOYERS_PER_BUCKET = 15;
const LABEL_PAD = 4;
type ViewState =
...
/**
 * Cap to top named employers per industry bucket (within each residence panel)
...
function mergeRareEmployersAcrossBuckets(rows: BguTreemapRow[]): BguTreemapRow[] {
  ...
  for (const panel of uniqueResidencePanels(rows)) {
    ...
    for (const [, bucketRows] of byBucket) {
      ...
      if (sorted.length <= TOP_EMPLOYERS_PER_BUCKET) {
        out.push(...bucketRows);
        continue;
      }
      const keep = new Set(sorted.slice(0, TOP_EMPLOYERS_PER_BUCKET).map(([e]) => e));
      for (const r of bucketRows) {
