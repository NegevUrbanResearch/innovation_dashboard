...
function nudgeLabelCentres(items: { x: number; y: number }[], vw: number, vh: number): void {
  ...
  const minDist = Math.max(32, 28 * scale);
...
function labelStrokeForFill(fill: string): string {
  const c = rgb(fill);
  const luminance = 0.299 * c.r + 0.587 * c.g + 0.114 * c.b;
  return luminance > 140 ? "rgba(255,255,255,0.92)" : "rgba(0,0,0,0.42)";
}
...
function appendSvgText(
  ...
  const stroke = labelStrokeForFill(fill);
  el.setAttribute("stroke", stroke);
  el.setAttribute("stroke-width", fontSizePx >= 12 ? "3" : "2.5");
...
export function mountBguCohortPartition(host: HTMLElement, data: CohortVennModel): () => void {
  ...
  function paint(): void {
    ...
    const labelFont = Math.max(
      11,
      lastW < 380 || lastH < 260 ? 11.5 : lastW < 480 || lastH < 310 ? 12.5 : 13.5,
    );
    ...
    for (const area of areas) {
      ...
      const tinySlice = area.size < nonemptyMax * 0.08 || pct < 0.02;
